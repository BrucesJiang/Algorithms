...
/**
 *  The {@code Inversion} class provides static methods to count the
 *  number of <em>inversion</em> in ether an array of integers or comparables
...
public class Inversion {
    ...
    private static long merge(int[] a, int[] aux, int lo, int mid, int hi){
        ...
        for(int k = lo; k <= hi; k ++){
            ...
            else if (less(aux[j], aux[i])){   a[k] = aux[j++]; inversion += (mid - i + 1);}
            else                              a[k] = aux[i++];
        }
        return inversion;
    }
    ...
    /**
     * Random generate a sequence of integers {@code N} and prints
     * the number of inversions to standard output
     *
